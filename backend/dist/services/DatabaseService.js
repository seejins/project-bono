...
class DatabaseService {
    constructor() {
        ...
        this.db = new pg_1.Client(dbConfig);
    ...
    async runMigrations() {
        ...
        try {
            ...
            await this.runMigration('allow_null_driver_id_in_edit_history', async () => {
                ...
                const columnInfo = await this.db.query(`SELECT column_name, is_nullable 
    ...
    async getAllSeasons() {
        const result = await this.db.query(`SELECT id, name, year, start_date, end_date, status, created_at, updated_at
    ...
    async createRace(data) {
        ...
        await this.db.query(`INSERT INTO races (id, season_id, track_id, track_name, race_date, status, created_at, updated_at) 
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`, [id, data.seasonId, data.trackId, trackName || 'Unknown Track', data.raceDate, data.status || 'scheduled', now, now]);
        return id;
    }
    async getRacesBySeason(seasonId) {
       ...
       ORDER BY r.race_date`, [seasonId]);
        ...
        const racesWithSessions = await Promise.all(result.rows.map(async (race) => {
            ...
            sessions.forEach(session => {
                ...
                if (sessionType >= 1 && sessionType <= 4) {
                    if (!sessionTypes.includes('practice'))
                        sessionTypes.push('practice');
                }
                else if (sessionType >= 5 && sessionType <= 9) {
                    if (!sessionTypes.includes('qualifying'))
                        sessionTypes.push('qualifying');
                }
                else if (sessionType === 10) {
                    if (!sessionTypes.includes('race'))
    ...
    async getRaceById(raceId) {
       ...
       WHERE r.id = $1`, [raceId]);
        ...
        const sessionResult = await this.db.query(`SELECT additional_data 
       ...
       LIMIT 1`, [raceId]);
        ...
        if (!totalLaps) {
            const maxLapsResult = await this.db.query(`SELECT MAX(num_laps) as max_laps 
    ...
    async importSessionResults(raceId, results) {
        ...
        for (const result of results) {
            const driverId = (result.driverId && String(result.driverId)) ||
                (result.driverName ? result.driverName : (0, uuid_1.v4)());
            const teamName = result.teamName ?? 'Unknown Team';
            const carNumber = result.carNumber !== undefined && result.carNumber !== null
                ? result.carNumber
                : 0;
            const position = result.position !== undefined && result.position !== null
                ? result.position
                : 0;
            const lapTime = result.lapTime !== undefined && result.lapTime !== null
                ? result.lapTime
                : null;
            const sector1 = result.sector1Time !== undefined && result.sector1Time !== null
                ? result.sector1Time
                : null;
            const sector2 = result.sector2Time !== undefined && result.sector2Time !== null
                ? result.sector2Time
                : null;
            const sector3 = result.sector3Time !== undefined && result.sector3Time !== null
                ? result.sector3Time
                : null;
            const bestLap = result.bestLapTime !== undefined && result.bestLapTime !== null
                ? result.bestLapTime
                : lapTime;
            await this.db.query(`INSERT INTO f123_session_results (id, race_id, driver_id, driver_name, team_name, car_number, position, lap_time, sector1_time, sector2_time, sector3_time, best_lap_time, created_at) 
    ...
    async storeF123SessionResults(raceId, sessionType, driverResults) {
        const now = new Date().toISOString();
        ...
        for (const result of driverResults) {
            ...
            await this.db.query(`INSERT INTO f123_session_results (
    ...
    async getDriverBySteamId(steamId) {
        const result = await this.db.query(`SELECT id, name, team, number, season_id as "seasonId", steam_id, is_active as "isActive", created_at as "createdAt", updated_at as "updatedAt" 
       FROM drivers WHERE steam_id = $1`, [steamId]);
        return result.rows[0] ? this.transformDriverToCamelCase(result.rows[0]) : null;
    }
    async findOrCreateTrack(trackName, lengthKm) {
        ...
        if (existing.rows[0]) {
            ...
            if (lengthKm && lengthKm > 0) {
                const currentLength = await this.db.query('SELECT length_km FROM tracks WHERE id = $1', [existing.rows[0].id]);
                if (!currentLength.rows[0]?.length_km || currentLength.rows[0].length_km === 0) {
                    await this.db.query('UPDATE tracks SET length_km = $1 WHERE id = $2', [lengthKm, existing.rows[0].id]);
        ...
        const now = new Date().toISOString();
        await this.db.query(`INSERT INTO tracks (id, name, country, length_km, created_at) 
         VALUES ($1, $2, $3, $4, $5)`, [id, trackName, 'Unknown', lengthKm || 0, now]);
        return id;
    }
    async getDriverMappings(seasonId) {
        return this.getDriverMappingsBySeason(seasonId);
    }
    async importRaceResults(raceId, data) {
        ...
        const normalizedResults = rawResults.map((result) => {
            ...
            return {
                ...
                fastestLap: typeof result.fastestLap === 'boolean'
                    ? result.fastestLap
                    : result.fastest_lap ?? false,
                polePosition: typeof result.polePosition === 'boolean'
                    ? result.polePosition
                    : result.pole_position ?? false,
                points: typeof result.points === 'number'
                    ? result.points
    ...
    async getMemberCareerProfile(memberId) {
    ...
    async getMemberRaceHistory(memberId, seasonId) {
        return this.getDriverRaceHistory(memberId, seasonId);
    }
    async createDriver(data) {
        const id = (0, uuid_1.v4)();
        const now = new Date().toISOString();
        await this.db.query(`INSERT INTO drivers (id, name, team, number, season_id, steam_id, is_active, created_at, updated_at) 
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`, [id, data.name, data.team || null, data.number || null, data.seasonId || null, data.steam_id || null, data.isActive ?? true, now, now]);
        return id;
    }
    ...
    async getDriverById(id) {
        const result = await this.db.query(`SELECT id, name, team, number, season_id as "seasonId", steam_id, is_active as "isActive", 
              created_at as "createdAt", updated_at as "updatedAt"
    ...
    async addUDPParticipant(data) {
        const now = new Date().toISOString();
        await this.db.query(`INSERT INTO f123_udp_participants (
        ...
        session_time, frame_identifier, created_at
            ...
            (0, uuid_1.v4)(), data.seasonId, data.driverId || null, data.vehicleIndex, data.aiControlled,
    ...
    async addUDPSessionResult(data) {
        const now = new Date().toISOString();
        await this.db.query(`INSERT INTO f123_udp_session_results (
        ...
        frame_identifier, created_at
            ...
            (0, uuid_1.v4)(), data.seasonId, data.eventId, data.driverId || null, data.position,
    ...
    async addUDPTyreStint(data) {
        const now = new Date().toISOString();
        await this.db.query(`INSERT INTO f123_udp_tyre_stints (
        ...
        tyre_visual_compound, session_uid, session_time, frame_identifier, created_at
            ...
            (0, uuid_1.v4)(), data.driverId || null, data.stintNumber, data.endLap,
    ...
    async addUDPLapHistory(data) {
        const now = new Date().toISOString();
        await this.db.query(`INSERT INTO f123_udp_lap_history (
        ...
        lap_valid_bit_flags, session_uid, session_time, frame_identifier, created_at
            ...
            (0, uuid_1.v4)(), data.driverId || null, data.lapNumber, data.lapTimeMs,
     ...
     */
    async batchAddUDPLapHistory(lapHistoryArray) {
        ...
        for (const data of lapHistoryArray) {
            placeholders.push(`($${paramIndex++}, $${paramIndex++}, $${paramIndex++}, $${paramIndex++}, $${paramIndex++}, ` +
                ...
                `$${paramIndex++}, $${paramIndex++}, $${paramIndex++}, $${paramIndex++}, $${paramIndex++})`);
            values.push((0, uuid_1.v4)(), data.driverId || null, data.lapNumber, data.lapTimeMs, data.sector1TimeMs, data.sector1TimeMinutes, data.sector2TimeMs, data.sector2TimeMinutes, data.sector3TimeMs, data.sector3TimeMinutes, data.lapValidBitFlags, data.sessionUid, data.sessionTime, data.frameIdentifier, now);
        }
        await this.db.query(`INSERT INTO f123_udp_lap_history (
        id, user_id, lap_number, lap_time_ms, sector1_time_ms, sector1_time_minutes,
        sector2_time_ms, sector2_time_minutes, sector3_time_ms, sector3_time_minutes,
    ...
    async getUDPLapHistory(driverId) {
        if (driverId) {
        const result = await this.db.query(`
    ...
    async getCurrentEventForSeason(seasonId) {
        const result = await this.db.query(`
    ...
    async getUDPParticipantsBySession(sessionUid) {
        const result = await this.db.query(`
    ...
    async getUDPSessionResultsBySession(sessionUid) {
        const result = await this.db.query(`
    ...
    async getUDPLapHistoryByDriver(driverId, sessionUid) {
        let query = `
      SELECT ulh.*, d.name as driver_name
      FROM f123_udp_lap_history ulh
      LEFT JOIN drivers d ON ulh.user_id = d.id
    ...
    async getUDPTyreStintsByDriver(driverId, sessionUid) {
        let query = `
    ...
    async addDriverToSeason(seasonId, driverId) {
        ...
        const existingDriver = await this.db.query('SELECT id FROM drivers WHERE season_id = $1 AND (id = $2 OR name = $3)', [seasonId, driverId, driver.name]);
        ...
        if (!driver.seasonId) {
            ...
            await this.db.query('INSERT INTO drivers (id, name, team, number, season_id, steam_id, is_active) VALUES ($1, $2, $3, $4, $5, $6, $7)', [driverId, driver.name, driver.team || 'TBD', driver.number || 0, seasonId, driver.steam_id || null, driver.isActive]);
        }
        else {
            // Update existing driver to include this season (or create duplicate for season-specific data)
            await this.db.query('UPDATE drivers SET season_id = $1 WHERE id = $2', [seasonId, driverId]);
        }
    ...
    async removeDriverFromSeason(seasonId, driverId) {
        ...
        await this.db.query(`UPDATE drivers
       SET season_id = NULL,
           team = NULL,
           number = NULL,
           updated_at = NOW()
       WHERE id = $1 AND season_id = $2`, [driverId, seasonId]);
        // Clear any driver mapping associations for this season
        await this.db.query(`UPDATE f123_driver_mappings
       SET your_driver_id = NULL,
           updated_at = NOW()
       WHERE season_id = $1 AND your_driver_id = $2`, [seasonId, driverId]);
        console.log(`✅ Driver ${driver.rows[0].name} removed from season ${seasonId}`);
    }
    async updateSeasonParticipant(driverId, data) {
        const updates = [];
        const values = [];
        let paramIndex = 1;
        if (data.team !== undefined) {
            updates.push(`team = $${paramIndex++}`);
            const normalizedTeam = typeof data.team === 'string'
                ? data.team.trim() || null
                : data.team ?? null;
        ...
        if (data.number !== undefined) {
            ...
            const normalizedNumber = data.number === undefined || data.number === null || Number.isNaN(data.number)
                ? null
                : data.number;
        ...
        await this.db.query(`UPDATE drivers SET ${updates.join(', ')} WHERE id = $${paramIndex}`, values);
    }
    ...
    async getEventsBySeason(seasonId) {
        const result = await this.db.query(`SELECT r.*, t.name as track_name_full, t.country, t.length_km 
       FROM races r 
       LEFT JOIN tracks t ON r.track_id = t.id 
       WHERE r.season_id = $1 
       ORDER BY r.race_date ASC`, [seasonId]);
        // Get sessions for each event to determine types
        const eventsWithSessions = await Promise.all(result.rows.map(async (row) => {
    ...
    async findActiveEventByTrack(trackName) {
        ...
        const result = await this.db.query(`SELECT id FROM races 
       WHERE track_name = $1 
    ...
    async getSessionByUID(sessionUID) {
        const result = await this.db.query(`SELECT sr.id, sr.session_name, r.track_name, r.race_date, sr.race_id
       FROM session_results sr 
       JOIN races r ON sr.race_id = r.id 
       WHERE sr.session_uid = $1 
       LIMIT 1`, [sessionUID.toString()]);
        if (result.rows.length === 0) {
            return null;
        }
    ...
    async createSessionResult(raceId, sessionType, sessionName, sessionUID, additionalData) {
        const now = new Date().toISOString();
        // Use ON CONFLICT to handle existing sessions - update completed_at and return existing ID
        const result = await this.db.query(`INSERT INTO session_results (id, race_id, session_type, session_name, session_uid, completed_at, created_at, additional_data) 
       ...
       RETURNING id`, [raceId, sessionType, sessionName, sessionUID, now, now, additionalData ? JSON.stringify(additionalData) : null]);
    ...
    async deleteDriverSessionResults(sessionResultId) {
        ...
        const driverResults = await this.db.query(`SELECT id FROM driver_session_results WHERE session_result_id = $1`, [sessionResultId]);
        const driverResultIds = driverResults.rows.map(r => r.id);
        if (driverResultIds.length > 0) {
            await this.db.query(`DELETE FROM lap_times WHERE driver_session_result_id = ANY($1)`, [driverResultIds]);
        }
        // Then delete driver session results (CASCADE will also delete lap_times, but we did it explicitly above)
        await this.db.query(`DELETE FROM driver_session_results WHERE session_result_id = $1`, [sessionResultId]);
    }
    // Store driver results for a session
    // Returns a map of driver result index to driver_session_result_id for linking lap times
    async storeDriverSessionResults(sessionResultId, driverResults) {
        const now = new Date().toISOString();
        const driverResultIdMap = new Map();
        for (let i = 0; i < driverResults.length; i++) {
            const result = driverResults[i];
            const driverSessionResultId = (0, uuid_1.v4)();
            await this.db.query(`INSERT INTO driver_session_results (
    ...
    async storeLapTimes(driverSessionResultId, raceId, lapData) {
        if (!lapData || lapData.length === 0) {
            return;
        }
        const now = new Date().toISOString();
        // Get user_id from driver_session_results for backward compatibility
        const driverResult = await this.db.query(`SELECT user_id FROM driver_session_results WHERE id = $1`, [driverSessionResultId]);
        const userId = driverResult.rows[0]?.user_id || null;
        // Batch insert lap times
        for (const lap of lapData) {
            await this.db.query(`INSERT INTO lap_times (
    ...
    async getCompletedSessions(raceId) {
        const result = await this.db.query(`SELECT id, session_type, session_name, completed_at, created_at
       FROM session_results 
       WHERE race_id = $1 
       ORDER BY session_type ASC`, [raceId]);
    ...
    async getDriverSessionResults(sessionResultId, includeLapTimes = false) {
        // First get the race_id and session_type from session_results
        const sessionInfo = await this.db.query(`SELECT sr.race_id, sr.session_type 
       FROM session_results sr 
       WHERE sr.id = $1`, [sessionResultId]);
        ...
        const countResult = await this.db.query(`SELECT COUNT(*) as count FROM driver_session_results WHERE session_result_id = $1`, [sessionResultId]);
       ...
       ORDER BY dsr.position ASC NULLS LAST`, [sessionResultId, raceId]);
    ...
    async deleteOriginalSessionResults(sessionResultId) {
        await this.db.query(`DELETE FROM session_results_original WHERE session_result_id = $1`, [sessionResultId]);
    }
    async storeOriginalSessionResults(sessionResultId, driverResults) {
        const now = new Date().toISOString();
        for (const result of driverResults) {
            await this.db.query(`INSERT INTO session_results_original (
          ...
          original_dnf_reason, created_at
                ...
                (0, uuid_1.v4)(), sessionResultId, result.user_id, result.position,
    ...
    async getSeasonIdFromEvent(eventId) {
    ...
    async recalculateDriverPostRacePenalties(driverSessionResultId) {
        const now = new Date().toISOString();
        const current = await this.db.query(`SELECT session_result_id, total_race_time_ms, base_race_time_ms
       FROM driver_session_results
       WHERE id = $1`, [driverSessionResultId]);
        if (!current.rows[0]) {
        ...
        if (baseRaceTimeMs == null) {
            ...
            await this.db.query(`UPDATE driver_session_results
         SET base_race_time_ms = $1
         WHERE id = $2`, [baseRaceTimeMs, driverSessionResultId]);
        }
        const penaltySum = await this.db.query(`SELECT COALESCE(SUM(seconds), 0) AS total_seconds
       FROM driver_penalties
       WHERE driver_session_result_id = $1`, [driverSessionResultId]);
       ...
       SET post_race_penalties = $1,
           ...
           updated_at = $3
       WHERE id = $4`, [totalPenaltySeconds, newTotalRaceTimeMs, now, driverSessionResultId]);
        return { sessionResultId };
    }
    // Retrieve penalties for a specific driver session result
    async getPenaltiesForDriverResult(driverSessionResultId) {
        const result = await this.db.query(`SELECT id, driver_session_result_id, seconds, reason, created_at, created_by
       FROM driver_penalties
       WHERE driver_session_result_id = $1
       ORDER BY created_at DESC`, [driverSessionResultId]);
        return result.rows;
    }
    // Retrieve penalties for all drivers in a session
    async getPenaltiesForSession(sessionResultId) {
        const result = await this.db.query(`SELECT dp.id,
       ...
       WHERE dsr.session_result_id = $1
       ORDER BY dp.created_at DESC`, [sessionResultId]);
        return result.rows;
    }
    ...
    async addPenalty(driverSessionResultId, penaltySeconds, reason, editedBy) {
        ...
        const insertResult = await this.db.query(`INSERT INTO driver_penalties (id, driver_session_result_id, seconds, reason, created_by)
       VALUES (gen_random_uuid(), $1, $2, $3, $4)
       RETURNING id, driver_session_result_id, seconds, reason, created_at, created_by`, [driverSessionResultId, normalizedSeconds, reason || null, editedBy || null]);
    ...
    async removePenalty(driverSessionResultId, penaltyId) {
        const deleted = await this.db.query(`DELETE FROM driver_penalties
       WHERE id = $1 AND driver_session_result_id = $2
       RETURNING driver_session_result_id`, [penaltyId, driverSessionResultId]);
    ...
    async recalculatePositions(sessionResultId) {
        // Get all drivers for this session with their total race times
        const drivers = await this.db.query(`SELECT id, position, total_race_time_ms, result_status
       FROM driver_session_results
       ...
       `, [sessionResultId]);
        ...
        for (let i = 0; i < drivers.rows.length; i++) {
            ...
            if (driver.position !== newPosition) {
                await this.db.query(`UPDATE driver_session_results 
           SET position = $1 
           WHERE id = $2`, [newPosition, driver.id]);
            }
        }
    }
    ...
    async changePosition(sessionResultId, driverId, newPosition, reason, editedBy) {
        const now = new Date().toISOString();
        // Get current values
        const current = await this.db.query('SELECT position FROM driver_session_results WHERE session_result_id = $1 AND driver_id = $2', [sessionResultId, driverId]);
        if (!current.rows[0]) {
            throw new Error('Driver session result not found');
        }
        const oldPosition = current.rows[0].position;
        // Update the result
        await this.db.query('UPDATE driver_session_results SET position = $1, updated_at = $2 WHERE session_result_id = $3 AND driver_id = $4', [newPosition, now, sessionResultId, driverId]);
        // Log the edit
        await this.db.query(`INSERT INTO race_edit_history (id, session_result_id, driver_id, edit_type, old_value, new_value, reason, edited_by, created_at)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`, [
            (0, uuid_1.v4)(), sessionResultId, driverId, 'position_change',
    ...
    async resetDriverToOriginal(sessionResultId, driverId) {
        const now = new Date().toISOString();
        // Get original values
        const original = await this.db.query('SELECT * FROM session_results_original WHERE session_result_id = $1 AND driver_id = $2', [sessionResultId, driverId]);
        if (!original.rows[0]) {
            throw new Error('Original session result not found');
        }
        const orig = original.rows[0];
        // Reset to original values
        await this.db.query(`UPDATE driver_session_results SET 
       ...
       WHERE session_result_id = $8 AND driver_id = $9`, [
        ...
        // Mark as restored
        await this.db.query('UPDATE session_results_original SET is_restored = true WHERE session_result_id = $1 AND driver_id = $2', [sessionResultId, driverId]);
        // Log the reset
        await this.db.query(`INSERT INTO race_edit_history (id, session_result_id, driver_id, edit_type, old_value, new_value, reason, edited_by, created_at)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`, [
            (0, uuid_1.v4)(), sessionResultId, driverId, 'reset_to_original',
            {}, {}, 'Reset to original UDP data', 'system', now
        ]);
    }
    // Get edit history for a session
    async getEditHistory(sessionResultId) {
        const result = await this.db.query(`SELECT reh.*, d.name as driver_name, dsr.json_driver_name, dsr.json_team_name
       ...
       ORDER BY reh.created_at DESC`, [sessionResultId]);
        return result.rows;
    }
    // Get edit history for a specific driver result (by driver_session_result_id)
    async getEditHistoryForDriver(driverSessionResultId) {
        const result = await this.db.query(`SELECT reh.*, d.name as driver_name, dsr.json_driver_name, dsr.json_team_name
       FROM race_edit_history reh
       LEFT JOIN drivers d ON reh.user_id = d.id
       LEFT JOIN driver_session_results dsr ON reh.driver_session_result_id = dsr.id
       WHERE reh.driver_session_result_id = $1
       ORDER BY reh.created_at DESC`, [driverSessionResultId]);
        return result.rows;
    }
    ...
    async revertEdit(editId) {
        const now = new Date().toISOString();
        // Get the edit
        const edit = await this.db.query('SELECT * FROM race_edit_history WHERE id = $1 AND is_reverted = false', [editId]);
        if (!edit.rows[0]) {
        ...
        if (editData.edit_type === 'penalty' || editData.edit_type === 'post_race_penalty' || editData.edit_type === 'post_race_penalty_removal') {
            ...
            if (editData.edit_type === 'post_race_penalty' || editData.edit_type === 'post_race_penalty_removal') {
                ...
                const driverResult = await this.db.query(`SELECT id FROM driver_session_results 
           WHERE session_result_id = $1 
           AND (driver_id = $2 OR (driver_id IS NULL AND $2 IS NULL))`, [editData.session_result_id, editData.driver_id]);
             ...
             WHERE id = $3`, [
                ...
                await this.db.query('UPDATE driver_session_results SET penalties = $1 WHERE session_result_id = $2 AND (driver_id = $3 OR (driver_id IS NULL AND $3 IS NULL))', [editData.old_value.penalties, editData.session_result_id, editData.driver_id]);
            }
        }
         ...
         AND (driver_id = $2 OR (driver_id IS NULL AND $2 IS NULL))`, [editData.session_result_id, editData.driver_id]);
            if (driverResult.rows[0]) {
                await this.db.query('UPDATE driver_session_results SET position = $1 WHERE id = $2', [editData.old_value.position, driverResult.rows[0].id]);
            }
        }
    ...
    async getDriverCareerProfile(driverId) {
        try {
            const driver = await this.getDriverById(driverId);
            if (!driver)
                return null;
    ...
    async getDriverSeasonStats(driverId, seasonId) {
        try {
    ...
    async getDriverRaceHistory(driverId, seasonId) {
        try {
            let query = `
        SELECT 
          r.id as race_id,
    ...
    async getPreviousRaceResults(seasonId) {
        try {
