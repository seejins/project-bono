...
exports.PostSessionProcessor = void 0;
class PostSessionProcessor {
    constructor(dbService, telemetryService, io, f123UDPProcessor) {
        this.f123UDPProcessor = null;
    ...
    async handleSessionEnd(finalResults) {
        try {
            ...
            const sessionResultId = await this.dbService.createSessionResult(eventId, sessionInfo.sessionType, this.dbService.getSessionTypeName(sessionInfo.sessionType), sessionInfo.sessionUID);
    ...
    extractSessionInfo(finalResults) {
    ...
    async findActiveEventBySession(sessionInfo) {
    ...
    async findFlexibleMatch(sessionInfo) {
    ...
    async mapDriversToLeague(finalResults, eventId) {
        ...
        const steamIdMappings = steamIds.length > 0 ? await this.dbService.query(`SELECT your_driver_id, f123_driver_name, f123_driver_number, f123_steam_id 
       FROM f123_driver_mappings 
       WHERE season_id = $1 AND f123_steam_id = ANY($2)`, [seasonId, steamIds]) : { rows: [] };
        const nameMappings = driverNames.length > 0 ? await this.dbService.query(`SELECT your_driver_id, f123_driver_name, f123_driver_number, f123_driver_name as driver_name
       FROM f123_driver_mappings 
    ...
    async handleOrphanedSession(sessionInfo, finalResults) {
    ...
    async notifyAdminOfOrphanedSession(sessionInfo) {
    ...
    async logSessionError(error, finalResults) {
...
exports.PostSessionProcessor = PostSessionProcessor;
//# sourceMappingURL=PostSessionProcessor.js.map