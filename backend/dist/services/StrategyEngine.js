...
class StrategyEngine {
    ...
    shouldPitStop(telemetry, lapsRemaining, weather) {
        const tireWear = this.calculateAverageTireWear(telemetry.tireWear);
        ...
        if (weather.rainPercentage > 30 && telemetry.rainPercentage < 10)
            return true;
    ...
    recommendTireCompound(weather, telemetry) {
        // Wet conditions
    ...
    calculateFuelStrategy(fuelRemaining, lapsRemaining) {
    ...
    analyzePace(telemetry) {
        ...
        if (paceDifference < 0.02)
            return 'fast';
        if (paceDifference < 0.05)
            return 'medium';
        return 'slow';
    }
    identifyIssues(telemetry) {
        const issues = [];
        // High tire wear
    ...
    generateReasoning(telemetry, needsPitStop, tireCompound, weather) {
        if (!needsPitStop) {
    ...
    calculateConfidence(telemetry, weather) {
        let confidence = 0.8; // Base confidence
        // Reduce confidence based on uncertainty factors
        if (weather.rainPercentage > 20)
            confidence -= 0.2;
        if (telemetry.lapNumber < 5)
            confidence -= 0.3; // Early in race
        if (this.lapHistory.length < 3)
            confidence -= 0.2; // Not enough data
