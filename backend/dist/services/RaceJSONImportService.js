...
class RaceJSONImportService {
        ...
        try {
            ...
            if (!eventName || eventName === 'Unknown') {
                ...
                if (trackNameMatches) {
                    ...
                    if (eventName.toLowerCase() === 'austria')
                        eventName = 'Austria';
                    else if (eventName.toLowerCase() === 'sakhir')
                        eventName = 'Sakhir (Bahrain)';
                    else if (eventName.toLowerCase() === 'texas')
                        eventName = 'Texas';
                    else if (eventName.toLowerCase() === 'brazil')
                        eventName = 'Brazil';
                    else if (eventName.toLowerCase() === 'mexico')
                        eventName = 'Mexico';
                    else if (eventName.toLowerCase() === 'baku')
                        eventName = 'Baku (Azerbaijan)';
                    else if (eventName.toLowerCase() === 'abu_dhabi' || eventName.toLowerCase() === 'abu dhabi')
                        eventName = 'Abu Dhabi';
                    else if (eventName.toLowerCase() === 'las_vegas' || eventName.toLowerCase() === 'las vegas')
                        eventName = 'Las Vegas';
                    else if (eventName.toLowerCase() === 'paul_ricard' || eventName.toLowerCase() === 'paul ricard')
                        eventName = 'Paul Ricard';
                }
            }
            // Import track name mapping to get full track name
            const { mapTrackIdToTrackName } = await Promise.resolve().then(() => __importStar(require('../utils/trackNameMapping')));
            ...
            const sessionInfo = F123JSONParser_1.F123JSONParser.parseSessionFile(filePath).sessionInfo;
            ...
            if (sessionUID !== null) {
                ...
                if (existingSession) {
                    throw new Error(`Session with UID ${sessionUID} already exists. ` +
            ...
            for (let i = 0; i < classificationData.length; i++) {
                ...
                if (!bestLapTimeMs && lapTimeHistory['lap-history-data'] && Array.isArray(lapTimeHistory['lap-history-data'])) {
                    ...
                    const bestLap = lapTimeHistory['lap-history-data'].reduce((best, lap) => {
                ...
                if (lapTimeHistory['lap-history-data'] && Array.isArray(lapTimeHistory['lap-history-data'])) {
                    ...
                    if (bestLapNum !== undefined && bestLapNum !== null) {
                        ...
                        bestLap = lapHistoryData.find((lap) => lap['lap-number'] === bestLapNum);
                    ...
                    if (!sector1TimeMs && bestS1Lap !== undefined && bestS1Lap !== null) {
                        let s1Lap = lapHistoryData.find((lap) => lap['lap-number'] === bestS1Lap);
                    ...
                    if (!sector2TimeMs && bestS2Lap !== undefined && bestS2Lap !== null) {
                        let s2Lap = lapHistoryData.find((lap) => lap['lap-number'] === bestS2Lap);
                    ...
                    if (!sector3TimeMs && bestS3Lap !== undefined && bestS3Lap !== null) {
                        let s3Lap = lapHistoryData.find((lap) => lap['lap-number'] === bestS3Lap);
                ...
                if (resultStatus === 4 || resultStatus === 5 || resultStatus === 6 || resultStatus === 7) { // DNF, DSQ, NCL, RET
                    dnfReason = (0, f123Constants_1.getResultStatus)(resultStatus);
                }
                ...
                if (i === 0) {
                    console.log(`📊 Extracted values for driver ${i + 1}:`, {
                        ...
                        lapHistoryDataLength: lapTimeHistory['lap-history-data'] ? lapTimeHistory['lap-history-data'].length : 0,
                ...
                const jsonTeamName = result?.team || // Team name from result (most reliable)
                    ...
                    for (let lapIndex = 0; lapIndex < lapHistoryData.length; lapIndex++) {
                        ...
                        if (lapNumber > 0 && lapTimeMs > 0) {
                            ...
                            if (additionalDriverData.perLapInfo && Array.isArray(additionalDriverData.perLapInfo)) {
                                perLapData = additionalDriverData.perLapInfo.find((pl) => (pl['lap-number'] || pl.lapNumber) === lapNumber);
                            }
                            // Extract tire compound
                            let tireCompound = undefined;
                            let tireAgeLaps = undefined;
                            if (perLapData) {
                            ...
                            if (!tireCompound && tyreStintsEndLaps.length > 0 && tyreStintsVisual.length > 0) {
                                ...
                                if (stintIndex < tyreStintsVisual.length) {
                                    ...
                                    if (typeof compound === 'string') {
                                        ...
                                        if (compoundLower.includes('soft'))
                                            tireCompound = 'S';
                                        else if (compoundLower.includes('medium'))
                                            tireCompound = 'M';
                                        else if (compoundLower.includes('hard'))
                                            tireCompound = 'H';
                                        else if (compoundLower.includes('intermediate'))
                                            tireCompound = 'I';
                                        else if (compoundLower.includes('wet'))
                                            tireCompound = 'W';
                                        else
            ...
            const sessionResultId = await this.dbService.createSessionResult(targetRaceId, sessionType, sessionTypeName, sessionUID, Object.keys(sessionAdditionalData).length > 0 ? sessionAdditionalData : undefined);
            ...
            for (let i = 0; i < driverResults.length; i++) {
                ...
                if (driverSessionResultId && driverResult._lap_times_data && driverResult._lap_times_data.length > 0) {
                    await this.dbService.storeLapTimes(driverSessionResultId, targetRaceId, driverResult._lap_times_data);
     ...
     */
    isFastestLap(classificationData, lapTimeMs) {
        if (lapTimeMs === 0)
            return false;
            ...
            if (!bestTime) {
                ...
                if (lth['lap-history-data'] && Array.isArray(lth['lap-history-data'])) {
                    const lapTimes = lth['lap-history-data']
                        .map((lap) => lap['lap-time-in-ms'] || lap.lapTimeInMs || 0)
                        .filter((time) => time > 0);
