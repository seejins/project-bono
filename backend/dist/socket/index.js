...
function setupSocketHandlers(io, services) {
    io.on('connection', (socket) => {
        ...
        socket.on('request_strategy', () => {
            const currentData = services.telemetryService.getLastData();
            if (currentData) {
                const strategy = services.strategyEngine.generateStrategy(currentData, 50, // Default race length
                {
    ...
    services.telemetryService.on('telemetry', (data) => {
        ...
        const strategy = services.strategyEngine.generateStrategy(data, 50, // Default race length
        {
