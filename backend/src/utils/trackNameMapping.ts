 ...
 */
export const trackIdToTrackName: { [key: string]: string } = {
 ...
 */
export function mapTrackIdToTrackName(trackId: string): string {
  if (!trackId) return 'Unknown Track';
  
  // Try exact match first
  if (trackIdToTrackName[trackId]) {
    return trackIdToTrackName[trackId];
  }
  
  // Try case-insensitive match
  const lowerTrackId = trackId.toLowerCase();
  for (const [key, value] of Object.entries(trackIdToTrackName)) {
