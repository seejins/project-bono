...
export default function createUploadRoutes(dbService: DatabaseService) {
  ...
  const storage = multer.diskStorage({
    ...
    filename: (req, file, cb) => {
      ...
      cb(null, `f123-session-${uniqueSuffix}${path.extname(file.originalname)}`);
    }
  });
  ...
  router.post('/session', upload.single('sessionFile'), async (req, res) => {
    try {
      ...
      const sessionData = F123Parser.parseSessionFile(req.file.path);
