...
const setupRacesRoutes = (
  ...
  io: Server,
  options?: { raceJsonImportService?: RaceJSONImportService }
) => {
  dbService = databaseService;
  raceResultsEditor = raceEditor;
  raceJSONImportService =
    options?.raceJsonImportService ?? new RaceJSONImportService(databaseService, io);
};
...
router.put('/driver-results/:driverSessionResultId/mapping', async (req, res) => {
  try {
    ...
    const updates = await raceResultsEditor.updateDriverUserMapping(
      driverSessionResultId,
      normalizedUserId,
      editedBy,
    ...
    if (errorCode === 'USER_ALREADY_MAPPED') {
      return res.status(409).json({
        ...
        conflictDriverSessionResultId: (error as any)?.conflictDriverSessionResultId
      });
    }
