...
export interface F123DriverData {
  ...
  dnf: boolean;
  dnfReason?: string;
  raceTime?: number; // in milliseconds
  gapToLeader?: number; // in milliseconds
...
export interface F123LapTime {
  ...
  sector1: number; // in milliseconds
  sector2: number; // in milliseconds
  sector3: number; // in milliseconds
  tireCompound: string;
}
...
export class F123Parser {
  /**
   * Parse F1 23 session file and extract race data
   */
  public static parseSessionFile(filePath: string): F123SessionData {
    try {
   ...
   */
  private static parseJSONFile(content: string): F123SessionData {
    ...
    // Extract driver data
    const drivers: F123DriverData[] = [];
    const results: F123RaceResult[] = [];
    
    if (data.drivers && Array.isArray(data.drivers)) {
      for (const driver of data.drivers) {
        const driverData: F123DriverData = {
   ...
   */
  private static parseCSVFile(content: string): F123SessionData {
   ...
   */
  private static detectSessionType(data: any): 'practice' | 'qualifying' | 'race' | 'sprint' {
   ...
   */
  private static extractLapTimes(lapTimesData: any[]): F123LapTime[] {
    if (!Array.isArray(lapTimesData)) return [];
   ...
   */
  private static convertTimeToMs(time: string | number): number {
   ...
   */
  public static validateSessionData(data: F123SessionData): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];
